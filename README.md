...
## Touchy (Featured Game) – Release Download Flow

The Featured "Download Game" button prefers an external GitHub Release asset URL when provided via env var, falling back to a local `public/downloads/featured-unity.7z`.

- Env var: `REACT_APP_TOUCHY_RELEASE_URL`
- Code: `src/assetsMap.js` reads this env var during build

### Create a Release and Upload Asset (PowerShell)

Prereqs: Git + GitHub CLI installed; run `gh auth login` once.

```powershell
# Example: upload a zipped/7z build and output the asset URL
./scripts/create-touchy-release.ps1 -AssetPath "C:\path\to\TouchyBuild.7z" -Tag touchy-v0.1.0 -Title "Touchy RPG v0.1.0"
```

The script will print the asset URL like:

```
https://github.com/<owner>/<repo>/releases/download/touchy-v0.1.0/TouchyBuild.7z
```

### Wire the Download Button to the Release Asset

Build with the env var set, then deploy:

```powershell
$env:REACT_APP_TOUCHY_RELEASE_URL = "https://github.com/<owner>/<repo>/releases/download/touchy-v0.1.0/TouchyBuild.7z"
npm run build
npx gh-pages -d build --dotfiles --message "Deploy Touchy release link"
```

Notes:
- Keep large archives out of `public/` to avoid GitHub Pages 100MB limit.
- You can update the same release tag with `--clobber`; clients will download the new asset URL when rebuilt.
