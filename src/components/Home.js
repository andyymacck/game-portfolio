...
const Home = () => {
  ...
  const [s2TddSize, setS2TddSize] = React.useState(null);
  const [s3GddSize, setS3GddSize] = React.useState(null);
  const [s1PdfExists, setS1PdfExists] = React.useState(true);
  const [s2PdfExists, setS2PdfExists] = React.useState(true);
  const [s2TddExists, setS2TddExists] = React.useState(true);
  const [s3PdfExists, setS3PdfExists] = React.useState(true);
  ...
  React.useEffect(() => {
    ...
    fetchSize(assets.slices?.s2?.tdd, setS2TddSize, setS2TddExists);
    fetchSize(assets.slices?.s3?.gdd, setS3GddSize, setS3PdfExists);
  }, []);
  const renderPdfLink = (href, label, size, exists, sliceId) => {
    // If no href provided or file missing, show an animated "Coming soon!" pill
    if (!href || !exists) {
      return (
        <span
          aria-live="polite"
      ...
      <div className="content-wrapper">
        ...
        <section className="showcase">
          ...
          <div className={`slices-grid ${activeSlice ? 'slice-activating' : ''}`}>
            {/* Slice 1 — Gears Shooter (Unreal/C++) */}
...
MovementOffsetYaw = UKismetMathLibrary::NormalizedDeltaRotator(MovementRotation, AimRotation).Yaw;`}</code></pre>
                    ...
                    <div style={{marginTop:'0.75rem'}}>
                      <span
                        aria-live="polite"
                        aria-label="Slice 1 GDD coming soon"
                        style={{
                          display:'inline-flex',
                          alignItems:'center',
                          gap:'0.45rem',
    ...
    while (true) {
      ...
      yield return null;
            ...
            <div className={`slice ${activeSlice === 's2' ? 'activating' : ''}`}>
              ...
              <h3>Bananaman - The Crystal Peel (Unity)</h3>
              <p className="slice-description">
                What if Pepsiman (PS1) was a 2.5D platformer? A heavily architected scalable event based low poly Unity 2.5D platformer slice inspired by games like Klonoa and Kirby 64. Players movement follows a spline system,
...
transform.position = pos;`}</code></pre>
                    <div style={{display:'flex',gap:'1rem',flexWrap:'wrap',justifyContent:'center',marginTop:'0.75rem'}}>
                      <div style={{display:'flex',flexDirection:'column',alignItems:'center',maxWidth:'560px',width:'100%'}}>
                        <img
                          src={process.env.PUBLIC_URL + '/unity/BananaManSpline.JPG'}
                          alt="Bananaman Spline Traversal"
                          style={{maxWidth:'560px',width:'100%',borderRadius:'8px',boxShadow:'0 2px 12px rgba(0,0,0,0.15)'}}
                        />
                        <p style={{fontSize:'0.95rem',marginTop:'0.5rem',color:'#3a3a3a',textAlign:'center'}}>
                          <span style={{fontWeight:'bold',color:'#222',fontSize:'1rem'}}>Spline‑guided pathing</span><br />
                          <span style={{fontSize:'0.9rem',color:'#444',fontWeight:'normal'}}>Player motion samples a Catmull‑Rom spline each frame (<code>GetPointAt(t)</code>) for smooth, camera‑friendly arcs. This lets set‑pieces curve and bank naturally while keeping inputs simple.</span>
                        </p>
                      </div>
                    </div>
                    ...
                    {/* New Additions */}
                    <h4 style={{marginTop:'1rem'}}>New: PSX Processing/Shader</h4>
                    <ul>
                      <li>Retro post‑process via <code>PSXPostProcess</code> and <code>Hidden/PSX/ColorDither</code>.</li>
                      <li>Tunables: palette steps, dither enable/intensity, per‑camera FOV integration.</li>
                    </ul>
                    <pre><code>{`void OnRenderImage(RenderTexture src, RenderTexture dst)
{
    ...
    Graphics.Blit(src, dst, m_ditherMaterial);
                    ...
                    <h4 style={{marginTop:'1rem'}}>New: Platform Moves on Player Collision</h4>
                    <ul>
                      <li>Platforms react to player contact and begin motion.</li>
                      <li>Player mounts cleanly via <code>PlatformRideEvent</code>, with velocity carry, stick forces, and offset lock to avoid drift.</li>
                    </ul>
                    <pre><code>{`void OnPlatformRide(PlatformRideEvent e)
{
    if (e.Player != this) return;
    m_ridingPlatform = e.IsMount;
    if (!m_ridingPlatform) m_hasRideOffset = false;
}`}</code></pre>
                    <pre><code>{`Vector3 platformPos = m_currentPlatform.transform.position;
Vector3 targetWorld = platformPos + m_platformOffsetPos; 
targetWorld.y = transform.position.y;
m_rigidbody.MovePosition(targetWorld);`}</code></pre>

                    <h4 style={{marginTop:'1rem'}}>New: Player Projectile</h4>
                    <ul>
                      <li>Ranged projectile system using <code>Bullet</code> with Rigidbody movement, optional homing, and target filters.</li>
                      <li>Safe spawn (ignores self‑collision), configurable speed/damage/visual scale, and unlockable via collectibles.</li>
                    </ul>
                    <pre><code>{`Vector3 chestBase = transform.position + Vector3.up * m_rangedChestHeight;
Vector3 dir = m_animationController.GetFacingDirection().normalized;
Bullet bullet = Instantiate(m_playerBulletPrefab, chestBase, Quaternion.LookRotation(dir));
...
bullet.ApplyMovement(dir);`}</code></pre>

                    <h4 style={{marginTop:'1rem'}}>New: Boulder Hazard Spawner</h4>
                    <ul>
                      <li>Timed/pooled spawning via <code>BoulderSpawner</code>/<code>RollingHazardSpawner</code>.</li>
                      <li>Hazards roll along paths or world space with configurable cadence, lifetime, and kill volumes.</li>
                    </ul>
                    <pre><code>{`// Timed spawn loop
...
void SpawnBoulder()
{
    var b = Instantiate(m_boulderPrefab, m_spawnPoint.position, m_spawnPoint.rotation);
              ...
              <div style={{marginTop:'0.5rem',display:'flex',gap:'0.5rem',flexWrap:'wrap'}}>
                ...
                {renderPdfLink(assets.slices.s2.tdd, 'View Slice 2 TDD', s2TddSize, s2TddExists, 'slice2-tdd')}
              </div>
            </div>
            {/* Slice 3 — Modern Platformer (Unreal/C++) */}
...
`}</code></pre>
              ...
              <div style={{marginTop:'0.5rem'}}>
                {renderPdfLink(assets.slices?.s3?.gdd, 'View Slice 3 GDD', s3GddSize, s3PdfExists, 'slice3')}
              </div>
            </div>
          </div>
        </section>
...
IEnumerator BurstFire() {
    for (int i = 0; i < 3; i++) {
        ...
        yield return new WaitForSeconds(0.5f);
                ...
                <h4>Summary</h4>
                <ul>
                  <li>Bosses aren’t just “tougher enemies”. Each are defined by unique programming logic that creates a distinct playstyle.</li>
            ...
            <div className="game-footer">
              ...
              <div className="game-footer-row">
                ...
                {assets.featured.download ? (
                  ...
                  <span
                    aria-live="polite"
                    aria-label="Featured game download coming soon"
                    style={{
                      display:'inline-flex',
                      alignItems:'center',
                      gap:'0.45rem',
